...
int M6800CoreInit(int num, int type)
{
	...
	for (int i = 0; i < num; i++) {
		...
		M6800CPUContext[i].ReadPort = M6800ReadByteDummyHandler;
		M6800CPUContext[i].WritePort = M6800WriteByteDummyHandler;
...
int M6800Scan(int nAction)
{
	...
	for (int i = 0; i < nM6800Count; i++) {
		...
		ba.Data = &M6800CPUContext[i];
		ba.nLen = sizeof(M6800CPUContext[i]);
		ba.szName = szName;
		BurnAcb(&ba);
	}
