...
static int LoadRoms()
{
	...
	memset(TmntSpriteTemp, 0, 0x200000);
	...
	memset(TmntSampleROM01, 0, 0x20000);
	...
	memset(TmntSampleROM02, 0, 0x20000);
	...
	memset(TmntSampleROM03, 0, 0x80000);
...
int TmntFrame()
{
	...
	for (int i = 1; i <=nInterleave; i++) {
		...
		int nNext;
		// Run 68000
		nNext = i *(nCyclesTotalSek / nInterleave);

		if (!CheckSleep(0)) {					// See if this CPU is busywaiting
			nCyclesDone[0] += SekRun(nNext - nCyclesDone[0]);
		} else {
			nCyclesDone[0] += nNext - nCyclesDone[0];
		}
		// Run Z80

		nNext = i * (nCyclesTotalZet / nInterleave);
...
static int MiaLoadRoms()
{
	...
	memset(TmntSpriteTemp, 0, 0x80000*2);
