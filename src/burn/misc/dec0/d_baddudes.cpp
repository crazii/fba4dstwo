...
//	int trans = 0;
	...
	/* Scroll positions */
	xscroll_b=(*(unsigned short*)&dec0_pf3_control_1[0]);
	yscroll_b=(*(unsigned short*)&dec0_pf3_control_1[2]);
	xscroll_f=(*(unsigned short*)&dec0_pf2_control_1[0]);
	yscroll_f=(*(unsigned short*)&dec0_pf2_control_1[2]);
	...
	if ((dec_pri_reg & 0x01) == 0)
	{
		...
		if ((*(unsigned short*)&dec0_pf2_control_1[6]) == 0x04)
		{
			for (offs = 0;offs < 16;offs++)
			{
				scrollx[offs] -= (*(unsigned short*)&dec0_pf2_rowscroll[2*offs]);
			}
		}
		scrolly=-yscroll_f;
		...
		if ((*(unsigned short*)&dec0_pf3_control_1[6]) == 0x04)
		{
			for (offs = 0;offs < 16;offs++)
			{
				scrollx[offs] -= (*(unsigned short*)&dec0_pf3_rowscroll[2*offs]);
