...
void pgm_dw2_decrypt()
{
	...
	for(int i=0; i<nPGM68KROMLen/2; i++) {
		...
		if (((i & 0x020890) == 0x000000) || ((i & 0x020000) == 0x020000 && (i & 0x001500) != 0x001400))
			x ^= 0x0002;

		if (((i & 0x020400) == 0x000000 && (i & 0x002010) != 0x002010) || ((i & 0x020000) == 0x020000 && (i & 0x000148) != 0x000140))
			x ^= 0x0400;
...
void pgm_killbld_decrypt()
{
	...
	for(int i = 0; i < 0x200000/2; i++) {
		...
		if ((i & 0x6d00) == 0x0400 || (i & 0x6c80) == 0x0880) x ^= 0x0008;
		if ((i & 0x7500) == 0x2400 || (i & 0x7600) == 0x3200) x ^= 0x1000;
...
static const unsigned char pstar_tab[256] = {
...
static const unsigned char theglad_tab[256] = { // IGS0005...
...
void pgm_theglad_decrypt()
{
	...
	for (int i = 0; i < 0x200000 / 2; i++) {
		...
		// not 100% sure, but does seem right...
		if ((i & 0x040080) != 0x000080) x ^= 0x0001;
		if ((i & 0x104008) == 0x104008) x ^= 0x0002;
		if ((i & 0x080030) == 0x080010) x ^= 0x0004;
		if ((i & 0x000042) != 0x000042) x ^= 0x0008;
...
static const unsigned char oldsplus_tab[256] = {
...
static const unsigned char kovshp_tab[256] = {
...
static const unsigned char killbldp_tab[256] = {
