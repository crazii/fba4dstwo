...
INLINE UINT32 sh2_GetTotalCycles()
{
...
INLINE int Sh2MapMemory(unsigned char* pMemory, unsigned int nStart, unsigned int nEnd, int nType)
{
...
INLINE int Sh2MapHandler(unsigned int nHandler, unsigned int nStart, unsigned int nEnd, int nType)
{
	unsigned char** pMemMap = pSh2Ext->MemMap + (nStart >> SH2_SHIFT);
	int need_mirror = (nStart < 0x08000000) ? 1 : 0;
...
INLINE int Sh2SetReadByteHandler(int i, pSh2ReadByteHandler pHandler)
{
...
INLINE int Sh2SetWriteByteHandler(int i, pSh2WriteByteHandler pHandler)
{
	if (i >= SH2_MAXHANDLER) return 1;
...
INLINE int Sh2SetReadWordHandler(int i, pSh2ReadWordHandler pHandler)
{
	if (i >= SH2_MAXHANDLER) return 1;
...
INLINE int Sh2SetWriteWordHandler(int i, pSh2WriteWordHandler pHandler)
{
	if (i >= SH2_MAXHANDLER) return 1;
...
INLINE int Sh2SetReadLongHandler(int i, pSh2ReadLongHandler pHandler)
{
	if (i >= SH2_MAXHANDLER) return 1;
...
INLINE int Sh2SetWriteLongHandler(int i, pSh2WriteLongHandler pHandler)
{
	if (i >= SH2_MAXHANDLER) return 1;
...
static unsigned char  __fastcall Sh2InnerReadByte(unsigned int a) 
{ 
	return sh2_internal_r((a & 0x1fc)>>2, ~(0xff << (((~a) & 3)*8))) >> (((~a) & 3)*8); 
}

static unsigned short __fastcall Sh2InnerReadWord(unsigned int a) 
{ 
	return sh2_internal_r((a & 0x1fc)>>2, ~(0xffff << (((~a) & 2)*8))) >> (((~a) & 2)*8); 
}

static unsigned int   __fastcall Sh2InnerReadLong(unsigned int a) 
{ 
	return sh2_internal_r((a & 0x1fc)>>2, 0); 
}

static void __fastcall Sh2InnerWriteByte(unsigned int a, unsigned char d) 
{ 
	//bprintf(0, _T("Attempt to write byte value   %02x to location %8x   offset: %04x\n"), d, a, (a & 0x1fc)>>2);
	sh2_internal_w((a & 0x1fc)>>2, d << (((~a) & 3)*8), ~(0xff << (((~a) & 3)*8)));
}

static void __fastcall Sh2InnerWriteWord(unsigned int a, unsigned short d) 
{ 
	sh2_internal_w((a & 0x1fc)>>2, d << (((~a) & 2)*8), ~(0xffff << (((~a) & 2)*8)));
}

static void __fastcall Sh2InnerWriteLong(unsigned int a, unsigned int d) 
{ 
	sh2_internal_w((a & 0x1fc)>>2, d, 0);
}

static unsigned char  __fastcall Sh2EmptyReadByte(unsigned int) { return 0xa5; }
static unsigned short __fastcall Sh2EmptyReadWord(unsigned int) { return 0xa5a5; }
static unsigned int   __fastcall Sh2EmptyReadLong(unsigned int) { return 0xa5a5a5a5; }
static void __fastcall Sh2EmptyWriteByte(unsigned int, unsigned char) { }
static void __fastcall Sh2EmptyWriteWord(unsigned int, unsigned short) { }
static void __fastcall Sh2EmptyWriteLong(unsigned int, unsigned int) { }
...
INLINE void Sh2Open(const int i)
{
	pSh2Ext = Sh2Ext + i;
	sh2 = & (pSh2Ext->sh2);
}

INLINE void Sh2Close()
{
}

INLINE int Sh2GetActive()
{
	return 0;
}

INLINE void Sh2Reset(unsigned int pc, unsigned r15)
{
...
INLINE unsigned char program_read_byte_32be(unsigned int /*A*/)
{
	return 0;	
}

INLINE unsigned short program_read_word_32be(unsigned int /*A*/)
{
	return 0;
}

INLINE unsigned int program_read_dword_32be(unsigned int /*A*/)
{
	return 0;
}

INLINE void program_write_byte_32be(unsigned int /*A*/, unsigned char /*V*/)
{
}

INLINE void program_write_word_32be(unsigned int /*A*/, unsigned short /*V*/)
{
}

INLINE void program_write_dword_32be(unsigned int /*A*/, unsigned int /*V*/)
{
}
...
INLINE void sh2_timer_resync(void)
{
...
INLINE void sh2_timer_activate(void)
{
...
INLINE void sh2_recalc_irq(void)
{
	int irq = 0, vector = -1;
	int  level;
...
INLINE void sh2_timer_callback()
{
	UINT16 frc;
//	int cpunum = param;
...
INLINE void sh2_dmac_callback(int dma)
{
//	cpuintrf_push_context(cpunum);
...
INLINE void sh2_dmac_check(int dma)
{
...
INLINE void sh2_internal_w(UINT32 offset, UINT32 data, UINT32 mem_mask)
{
...
INLINE UINT32 sh2_internal_r(UINT32 offset, UINT32 mem_mask)
{
...
INLINE void Sh2SetIRQLine(const int line, const int state)
{
...
INLINE unsigned int Sh2GetPC(int)
{
	return (sh2->delay) ? (sh2->delay & AM) : (sh2->pc & AM);
}

INLINE void Sh2SetVBR(unsigned int i)
{
	sh2->vbr = i;
}

INLINE void Sh2BurnUntilInt(int)
{
	pSh2Ext->suspend = 1;
}

INLINE void Sh2StopRun()
{
	sh2->sh2_icount = 0;
...
static void __fastcall Sh2WriteByte(unsigned int a, unsigned char d)
{
	WB(a, d);
}


static unsigned char __fastcall Sh2ReadByte(unsigned int a)
{
	return RB(a);
}
...
static int Sh2Scan(int nAction)
{
