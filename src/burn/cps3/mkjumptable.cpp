...
main()
{
	...
	ot("UINT32 temp, temp0, temp1, temp2, temp3;\n");
	ot("INT32 tempm, tempn, fnLmL, templ;\n");
        ot("INT32 HH, HL, LH, LL;\n");
        ot("INT32 dest, src, ans, imm;\n");
        ot("\n");
	ot("UINT32 tmp0, tmp1;\n");
	ot("UINT32 old_q;\n");
        ot("\n");
	...
	fclose(OpCodeFile);
        OpCodeFile = fopen("sh2_jumptable.c","wt"); if (OpCodeFile==NULL) return 1;
        printf("Making sh2_jumptable.inc ...\n");
	ot("\n// --- Jump Table ---------------------------------\n\n");
	ot("static void* sh2JumpTable[0x10000] = {\n");
		...
		if ((i & 7) == 0) {
			if (jtb[i] == -1)
			...
			else				
                        {
                                ...
                                if(k==i)
                                        sprintf(buf, "\t\t&&Op%04X", jtb[i] );
                                else 
                                        sprintf(buf, "\t\(void*)0xFEDC%04X",k);
                        }
		} else {
			if (jtb[i] == -1)
                        {
                        	strcat(buf, ",\t(void*)0xFEDC0000");
                        }
			else {				
				for(int k=0;k<i;k++)
                                {
				        ...
				        sprintf(buf2, ",\t(void*)0xFEDC%04X", k );
                                strcat(buf, buf2);	
			}
		}
		if ((i & 7) == 7)		ot("%s,\t\t// %04x\n", buf, i );
