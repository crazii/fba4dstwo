...
static INT32 get_sample(ics2115_voice& voice)
{
	...
	if (voice.osc_conf.bitflags.eightbit) {
#if defined NDS && !defined DISABLE_SND_CACHE
		m_rom=getBlockSmallData(nPGMSNDROMOffset+curaddr);
		if(m_rom)
		{
			sample1 = ((INT8)m_rom[0]) << 8;
			sample2 = ((INT8)m_rom[1]) << 8;
		}
		else
			sample1 = sample2 = 0;
#else
		...
		if(m_rom)
		{
			sample1 = m_rom[0] | (((INT8)m_rom[1]) << 8);
			if(next)
				m_rom+=2;
			else
				m_rom=getBlockSmallData(nPGMSNDROMOffset+nextaddr);

			if(m_rom)
				sample2 = m_rom[0] | (((INT8)m_rom[1]) << 8);
			else
				sample1 = sample2 = 0;
		}
		else
...
void ics2115_update(INT16 *outputs, int samples)
{
	...
	for(int osc = 0; osc <= m_active_osc; osc++) {
		...
		if(ics2115_fill_output(voice, outputs ? stream : NULL, sample_per_frame))
			irq_invalid = true;
	}
...
void ics2115_recalc_irq()
{
	...
	m_irq_on = irq;
	if(m_irq_cb)
		m_irq_cb(irq ? ZET_IRQSTATUS_ACK : ZET_IRQSTATUS_NONE);
}
